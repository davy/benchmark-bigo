...
class TestChart < MiniTest::Test
  ...
  def test_comparison_for
    ...
    comparison = [{:data=>{1000=>1, 2000=>2, 3000=>3}}, {:name=>"const", :data=>{1000=>1.0, 2000=>1.0, 3000=>1.0}}, {:name=>"log n", :data=>{1000=>1.0, 2000=>1.1003433318879936, 3000=>1.1590404182398875}}, {:name=>"n", :data=>{1000=>1.0, 2000=>2.0, 3000=>3.0}}, {:name=>"n log n", :data=>{1000=>1.0, 2000=>2.200686663775987, 3000=>3.4771212547196626}}, {:name=>"n squared", :data=>{1000=>1.0, 2000=>4.0, 3000=>9.0}}, {:name=>"polynomial", :data=>{1000=>1.0, 2000=>Float::INFINITY, 3000=>Float::INFINITY}}]
  ...
  def test_poly_type
    assert Benchmark::BigO::Chart::TYPES.include? :poly

    assert_equal "polynomial", @chart.title_for(:poly)
  end
